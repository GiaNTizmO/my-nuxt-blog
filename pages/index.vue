...
// Получаем последние 3 поста из папки content/posts

const { data: posts } = await useAsyncData(() => queryCollection('content').path('/').all())
</script>
...
<template>
  ...
  <main>
    ...
    <section class="text-center py-20">
      <h1 class="text-4xl font-bold mb-4">Payload Website Template</h1>
      <p class="mb-6">The code for this template is completely open-source and can be found on our Github.</p>
      <div class="space-x-4">
    ...
    <section class="container mx-auto py-20">
      <img src="/images/image-hero1.webp" alt="Feature Image" class="w-full rounded mb-12" />
        ...
        <div class="grid md:grid-cols-3 gap-8">
          <Card v-for="post in posts" :key="post.path" class="p-6">
            <h3 class="text-xl font-bold mb-2">
              <nuxt-link :to="`/posts/${post.slug}`" class="hover:underline">{{ post.title }}</nuxt-link>
            </h3>
  ...
  <footer class="bg-white border-t py-6">
    <div class="container mx-auto text-center">
      ...
      <p class="text-gray-600 mt-2">Theme Admin</p>
    </div>
  </footer>
</template>
